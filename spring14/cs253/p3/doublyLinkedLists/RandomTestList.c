...
void runRandomTests(int count, unsigned int seed, int n, ListPtr list)
{
	...
	for (i = 0; i < count; i++) {
		...
		case 2:
			if (DEBUG > 1)
				fprintf(stderr, "%d. removeFront\n", i);
			node = removeFront(list);
			freeNode(node);
			break;
		case 3:
			if (DEBUG > 1)
				fprintf(stderr, "%d. removeRear\n", i);
			node = removeRear(list);
			freeNode(node);
			break;
		case 4:
			if (DEBUG > 1)
				fprintf(stderr, "%d. removeNode\n", i);
			node = removeNode(list, search(list, i));
			freeNode(node);
			break;
		case 5:
			if (DEBUG > 1)
				fprintf(stderr, "%d. reverseList\n", i);
			reverseList(list);
			break;
		case 6:
			if (DEBUG > 1)
				fprintf(stderr, "%d. searchList\n", i);
			node = search(list, i);
			break;
		default:
			break;
