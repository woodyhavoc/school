...
{
	ObjectPtr obj = createObject(jobid, "cmd args");
	NodePtr node = createNode(obj);
	return node;
}
...
Boolean searchWithNullList()
{
	...
	NodePtr nodeTest = search(NULL, node->obj);
...
Boolean searchWithEmptyList()
{
	...
	NodePtr nodeTest = search(testlist, node->obj);
...
Boolean searchOneNodeNotInList()
{
	...
	NodePtr nodeTest = search(testlist, node1->obj);
