...
/**
 * Checks each background job in the list to see which
 * of them have been completed.  When one of them is complete,
 * a message is display saying as much, along with the command
 * of the job, and its job number.
 *
 * @param list The list of background jobs to be checked.
 */
void checkComplete(ListPtr list);

/**
 * Returns the revison number of the subversion
 * repository.
 */
int svn_version();

/**
 * Main function, creates a ListPtr object, then enters a while
 * loop which continues until the user enters the command "exit"
 * or they press CTRL+D.
 *
 * @param argc Command-line argument count
 * @param argv Contains the actual command-line arguments
 */
int main(int argc, char** argv)
{
	if(checkSvn(argc, argv))
	{
		exit(0);
	}
...
Boolean checkSvn(int argc, char** argv)
{
	...
	{
		printf("Subversion revision number: %d\n", svn_version());
...
Boolean evalCmd(CommandPtr command, ListPtr jobList)
{
	...
	if(!strcmp(command->tokens[0], "jobs"))
	{
		if(jobList->head != NULL)
		{
			...
			pid_t jobPid;
			int count = 0;
			int size = getSize(jobList);
			
			while(count < size)
			{
				job = (JobPtr) node->obj;

				jobPid = waitpid(job->pid, &status, WNOHANG);

				if(jobPid)
				{
					printf("[%u] Done %s\n", job->jobNum, job->cmd);
					node = node->next;
					NodePtr tmp = removeNode(jobList, node->prev);
					freeObject(tmp->obj);
					free(tmp);
				}
				else if(!wasReported(job))
				{
					printf("[%u] Running %s\n", job->jobNum, job->cmd);
					node = node->next;
					setReported(job);
	...
	if(!strcmp(command->tokens[0], "exit"))
	{
		return TRUE;
	}
	
	if(!strcmp(command->tokens[0], "cd"))
	{
		if(command->tokNum == 1)
		{
...
void checkComplete(ListPtr list)
{
	...
	if(list->head != NULL)
	{
		...
		while(count < size)
		{
			...
			jobPid = waitpid(job->pid, 0, WNOHANG);

			if(jobPid)
			{
				printf("[%u] Done %s\n", job->jobNum, job->cmd);
				node = node->next;
				NodePtr tmp = removeNode(list, node->prev);
				freeObject(tmp->obj);
				free(tmp);
				count++;
				continue;
